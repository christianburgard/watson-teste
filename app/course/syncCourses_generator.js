...
function syncCourses(app) {
    ...
    function run(generator) {
        ...
        /**
         * retorna promise, podendo fazer validações da response recebida
         * @param {*resp de http.get} resp 
         */
        var processData=(resp)=>{
            ...
            resp.pipe(concat((data) => {
                // data = buffer totalmente carregado!
                var classrooms_json = JSON.parse(data);

                if(classrooms_json.length < 2) {
                    return it.throw('Dados inválidos! (Array vazia)');
                }
        ...
        function done(retorno) {
            ...
            if(retorno === -1) {
                return http.get(options, (resp) => {
                    var ret=it.next(processData(resp));
                }).on("error", (err) => {
